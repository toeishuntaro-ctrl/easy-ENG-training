...
function synthesizeRelatedPhrases(target, keyPattern, keyPatternJp) {
  const p = (keyPattern || '').toLowerCase();
  const t = (target || '').toLowerCase();
  
  if (p.includes('see your point') || p.includes('understand') || p.includes('hear you') || p.includes('fair point')) {
    return [
      {
        phrase_en: "I hear what you're saying, but our priority is to protect deliverable quality.",
        phrase_jp: "言わんとすることは分かりますが、成果物の品質確保が最優先です。",
        nuance_tag: "率直・要点明確"
      },
      {
        phrase_en: "That makes sense, but we should make sure we have all the facts first.",
        phrase_jp: "ごもっともですが、まずは全容と事実関係を確認しておくべきです。",
        nuance_tag: "クッション＋客観的"
      },
      {
        phrase_en: "I agree in principle, but we have to work within our current resource limits.",
        phrase_jp: "大筋では賛同しますが、現行のリソースの範囲内で進める必要があります。",
        nuance_tag: "大人の外交的合意"
      }
    ];
  } else if (p.includes('check with') || p.includes('get back') || p.includes('looking into') || p.includes('double-check')) {
    return [
      {
        phrase_en: "Let me verify the details and ping you back by this afternoon.",
        phrase_jp: "詳細を確認し、本日の午後までにチャットで折り返します。",
        nuance_tag: "口頭・即レス向き"
      },
      {
        phrase_en: "I will look into the root cause and share an update within two hours.",
        phrase_jp: "根本原因を調査の上、2時間以内に最新状況を共有します。",
        nuance_tag: "コミット・安心感付与"
      },
      {
        phrase_en: "Give me a short window to confirm with our team leads before we decide.",
        phrase_jp: "決定する前に、リーダー陣に確認する少しの時間をください。",
        nuance_tag: "角を立てずに時間確保"
      }
    ];
  } else if (p.includes('would it be') || p.includes('possible to') || p.includes('could we') || p.includes('can we')) {
    return [
      {
        phrase_en: "Would it make sense to deliver the critical parts first?",
        phrase_jp: "まずは重要パーツだけ先行して納品するのはいかがでしょうか？",
        nuance_tag: "よりソフト・問いかけ型"
      },
      {
        phrase_en: "A practical workaround is to split the milestone into two phases.",
        phrase_jp: "現実的な回避策として、マイルストーンを2段階に分ける手があります。",
        nuance_tag: "プロフェッショナル提案"
      },
      {
        phrase_en: "Could we agree on the essential scope first and defer the rest?",
        phrase_jp: "まずは必須範囲を確定させ、残りは後回しにすることは可能でしょうか？",
        nuance_tag: "合意形成・スコープ調整"
      }
    ];
  } else if (p.includes('need to') || p.includes('have to') || p.includes('should')) {
    return [
      {
        phrase_en: "To keep our commitments, let's take a practical step forward together.",
        phrase_jp: "約束を守るため、現実的な一歩を踏み出しましょう。",
        nuance_tag: "合意重視・推進"
      },
      {
        phrase_en: "We need to align on the core requirements before we proceed with development.",
        phrase_jp: "開発に着手する前に、コア要件の認識を合わせる必要があります。",
        nuance_tag: "手戻り防止・着実"
      },
      {
        phrase_en: "Our safest path forward is to freeze specs by end of day today.",
        phrase_jp: "最も安全な進め方は、本日中に仕様を確定させることです。",
        nuance_tag: "リスク回避・断定"
      }
    ];
  } else if (p.includes('could you please') || p.includes('would you be able') || p.includes('please let us know')) {
    return [
      {
        phrase_en: "Could you send across the latest deck so we can review it before the call?",
        phrase_jp: "コールの前に確認できるよう、最新資料をお送りいただけますか？",
        nuance_tag: "事前準備・丁寧"
      },
      {
        phrase_en: "Would you be able to give us a green light by 3 PM today?",
        phrase_jp: "本日15時までにゴーサインをいただくことは可能でしょうか？",
        nuance_tag: "期日明示・プッシュ"
      },
      {
        phrase_en: "Please keep us in the loop once the decision is finalized.",
        phrase_jp: "決定が下り次第、情報共有をいただけますと幸いです。",
        nuance_tag: "フォロー依頼"
      }
    ];
  } else if (p.includes('concern') || p.includes('risk') || p.includes('avoid') || p.includes('prevent')) {
    return [
      {
        phrase_en: "My main concern is timeline slippage, so let's set an interim checkpoint.",
        phrase_jp: "一番の懸念はスケジュールの遅延なので、中間チェックポイントを設けましょう。",
        nuance_tag: "先回り防止"
      },
      {
        phrase_en: "To avoid any misunderstanding, let's summarize the key takeaways in writing.",
        phrase_jp: "誤解を防ぐため、要点を書面でまとめておきましょう。",
        nuance_tag: "透明性・手戻り防止"
      },
      {
        phrase_en: "There is a slight risk on capacity, so let's prepare a fallback option.",
        phrase_jp: "稼働に若干のリスクがあるため、代替プランを準備しておきましょう。",
        nuance_tag: "リスク管理・備え"
      }
    ];
  } else if (p.includes('lock in') || p.includes('stick to') || p.includes('focus on') || p.includes('same page')) {
    return [
      {
        phrase_en: "Let's stick to the agreed scope to make sure we hit the hard deadline.",
        phrase_jp: "厳格な納期を守るため、合意済みのスコープを維持しましょう。",
        nuance_tag: "スコープ死守"
      },
      {
        phrase_en: "I want to make sure we stay aligned before taking the next step.",
        phrase_jp: "次のステップに進む前に、認識のズレがないことを確認したいです。",
        nuance_tag: "丁寧・すり合わせ"
      },
      {
        phrase_en: "Let's lock in Friday noon as our cutoff for all final feedback.",
        phrase_jp: "最終フィードバックの締め切りを金曜正午として確定させましょう。",
        nuance_tag: "期日固定・明快"
      }
    ];
  } else if (p.includes('thank you') || p.includes('thanks') || p.includes('appreciate')) {
    return [
      {
        phrase_en: "Thank you for the prompt turnaround; this really helps keep us on track.",
        phrase_jp: "迅速にご対応いただきありがとうございます。予定通りの進行に大変助かります。",
        nuance_tag: "感謝・信頼構築"
      },
      {
        phrase_en: "Thanks for your flexibility on adjusting to these new requirements.",
        phrase_jp: "新たな要件への柔軟なご調整、誠にありがとうございます。",
        nuance_tag: "協調姿勢への感謝"
      },
      {
        phrase_en: "I appreciate your partnership in finding a practical compromise.",
        phrase_jp: "現実的な落としどころを一緒に見出してくださり感謝いたします。",
        nuance_tag: "パートナーシップ強化"
      }
    ];
  } else {
    return [
      ...
      {
        phrase_en: "Let me follow up with a clear action plan shortly.",
        phrase_jp: "まもなく明確なアクションプランをもってフォローします。",
        nuance_tag: "スピード・信頼獲得"
      }
    ];
  }
}
...
`;
  ...
  // Models priority list: stable 2.5-flash, 2.5-flash-lite, 3.1-flash-lite
  const modelsToTry = [
    'gemini-2.5-flash',
      ...
      try {
        ...
        const formattedScenarios = scenarios.map((sc, idx) => {
          ...
          return {
            ...
            related_phrases: (Array.isArray(sc.related_phrases) && sc.related_phrases.length >= 2)
              ? sc.related_phrases
              : synthesizeRelatedPhrases(target, sc.key_pattern, sc.key_pattern_jp),
